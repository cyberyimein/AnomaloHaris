...
def _client(
    ...
    *,
    ...
    monkeypatch.setattr("app.api.copilot_hooks.get_buddy_gateway", lambda: gateway)
    monkeypatch.setattr(
        "app.api.copilot_hooks.get_settings",
...
def test_pre_tool_use_restores_coding_state(monkeypatch) -> None:
    ...
    response = client.post(
        ...
        json={
            ...
            "timestamp": 1234,
            "toolName": "bash",
            "toolArgs": {"command": "pytest -q tests/test_buddy_api.py"},
        },
    )

    assert response.status_code == 200
    assert response.json() == {}
    assert gateway.state_calls == [("coding", "bash: pytest -q tests/test_buddy_api.py")]
...
def test_permission_request_uses_buddy_approval_when_bridge_enabled(monkeypatch) -> None:
    ...
    assert gateway.approval_calls[0][1] == "Allow bash: pytest -q tests/test_buddy_api.py"
