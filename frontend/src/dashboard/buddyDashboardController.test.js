...
function createController(overrides = {}) {
  const requestJson = vi.fn(async (url) => {
    if (url === "/api/buddy/status") {
      return { connected: true, transport: "tcp", client_address: "192.0.2.20", recent_event_count: 1 };
    }
...
describe("BuddyDashboardController", () => {
  it("refreshes one coherent Buddy snapshot", async () => {
    ...
    expect(controller.state.statusCards.value).toContainEqual({ label: "Transport", value: "tcp" });
  ...
  it("uses the expected command endpoint and reloads events", async () => {
    const { controller, requestJson } = createController();
    ...
    expect(requestJson).toHaveBeenCalledWith(
      "/api/buddy/state",
      expect.objectContaining({ method: "POST", body: JSON.stringify({ state: "thinking" }) }),
    );
    expect(requestJson.mock.calls.filter(([url]) => url === "/api/buddy/events?limit=30")).toHaveLength(1);
  });

  it("reports failures, marks access errors, and clears busy state", async () => {
  ...
  it("starts and stops one polling timer", () => {
    const scheduler = { setInterval: vi.fn(() => 42), clearInterval: vi.fn() };
...
describe("Buddy event projection", () => {
  it("flattens nested event details with readable labels and values", () => {
    expect(buddyEventDetails({ payload: { call_buddy: { online: true }, choices: ["approve", "deny"], empty: null } })).toEqual([
  ...
  it("caps pathological payloads at 48 details", () => {
    const payload = Object.fromEntries(Array.from({ length: 60 }, (_, index) => [`field_${index}`, index]));
