...
export function createBuddyDashboardController({
  ...
  const visionHint = computed(() => {
    const status = visionStatus.value;
    if (!status) {
  ...
  async function startVision() {
    await runAction("Starting face detection...", async () => {
      visionStatus.value = await requestJson("/api/buddy/vision/start", { method: "POST" });
    });
  }

  async function setVisionEnabled(enabled) {
    await runAction(`${enabled ? "Enabling" : "Disabling"} Vision...`, async () => {
      visionStatus.value = await requestJson(
        enabled ? "/api/buddy/vision/enable" : "/api/buddy/vision/disable",
        { method: "POST" },
      );
    });
  }
