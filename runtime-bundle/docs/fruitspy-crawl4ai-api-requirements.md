...
调用方：AnomaloHaris Agent Backend
服务方：Fruitspy
目标版本：v1

## 1. 目标

Fruitspy 提供一个受控的 Crawl4AI HTTP API，为 AnomaloHaris 的 `web_fetch` 工具处理需要
JavaScript 渲染、浏览器执行或更强正文提取能力的公开网页。

职责边界：

- AnomaloHaris 负责搜索、工具路由、LLM 上下文长度控制、会话 trace 和开发侧栏展示。
...
如果 Direct Fetch 得到了可用内容，但 Crawl4AI 暂时失败，AnomaloHaris 会保留 Direct Fetch
...
- 当前生产环境中 Fruitspy 地址为宿主机可达地址，不应假设与 AnomaloHaris 在同一容器。
...
AnomaloHaris 当前也接受响应包裹在 `data` 中：
...
以下字段 AnomaloHaris 当前不会依赖，可以安全增加，便于后续评测：
...
兼容说明：AnomaloHaris 当前会把非 2xx 响应视为工具失败。对于 HTTP 200 响应，如果顶层或
...
第一版不要求 Fruitspy 截断 Markdown。AnomaloHaris 会按 `WEB_FETCH_MAX_CHARS` 截断并通过
`start_char` 分页。不过 Fruitspy 的完整 JSON 响应必须小于 AnomaloHaris 配置的
...
- 不自动登录、不处理验证码、不绕过付费墙；
- 不携带 AnomaloHaris 用户的 Cookie、Authorization 或浏览器会话；
...
AnomaloHaris 会在请求前和收到 `final_url` 后再次校验，但 Fruitspy 仍必须完成上述检查。
...
状态接口不是当前 AnomaloHaris 客户端的硬依赖，但建议与 Fruitspy Python Sandbox 的 status
风格保持一致。
...
- 页面不能读取 Fruitspy 或 AnomaloHaris 的环境变量和文件系统。

### 11.4 AnomaloHaris 端到端

在 AnomaloHaris 容器中执行：
...
- 超时、并发和响应大小有硬限制；
- 在 macmini 部署后，AnomaloHaris `auto` 模式端到端验证通过；
- AnomaloHaris Web Activity 能显示 `crawl4ai`、耗时、HTTP 状态、rendered 和 Markdown。
