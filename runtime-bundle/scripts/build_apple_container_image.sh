...
usage() {
    cat <<'EOF'
Build AnomaloHaris with Apple container and save the image as an OCI tar archive.
...
Environment:
  ...
  IMAGE_NAME               Local image name. Default: anomaloharis
  ...
  OUTPUT_DIR               Archive output directory. Default: runtime-bundle/artifacts/container-images
  ARCHIVE_PATH             Exact output archive path. Overrides OUTPUT_DIR naming
  DOCKERFILE               Dockerfile path. Default: runtime-bundle/docker/anomaloharis/Dockerfile
...
IMAGE_NAME="${IMAGE_NAME:-anomaloharis}"
PLATFORM="${PLATFORM:-linux/arm64}"
OUTPUT_DIR="${OUTPUT_DIR:-$agent_backend_root/artifacts/container-images}"
DOCKERFILE="${DOCKERFILE:-$agent_backend_root/docker/anomaloharis/Dockerfile}"
