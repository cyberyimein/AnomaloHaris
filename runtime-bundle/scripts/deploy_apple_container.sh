...
usage() {
    cat <<'EOF'
Copy a saved AnomaloHaris Apple container image to a remote Mac and run it there.

Usage:
  REMOTE=mac-mini.local scripts/deploy_apple_container.sh artifacts/container-images/anomaloharis-<tag>-linux-arm64.env
  REMOTE=mac-mini.local IMAGE_REF=anomaloharis:<tag> scripts/deploy_apple_container.sh artifacts/container-images/anomaloharis-<tag>-linux-arm64.tar
...
Environment:
  ...
  CONTAINER_NETWORK       Dedicated container network. Default: anomaloharis-external
EOF
}
...
if [[ -n "${REMOTE_DATA_DIR:-}" ]]; then
    selected_remote_data_dir="$REMOTE_DATA_DIR"
else
    selected_remote_data_dir="__ANOMALOHARIS_AUTO_DATA_DIR__:$legacy_remote_data_dir:$default_remote_data_dir"
fi
REMOTE_CONTAINER_CLI="${REMOTE_CONTAINER_CLI:-container}"
CONTAINER_NAME="${CONTAINER_NAME:-anomaloharis}"
...
REMOTE_ENV_FILE="${REMOTE_ENV_FILE:-$REMOTE_DIR/anomaloharis.env}"
...
if [[ "$remote_data_dir" == __ANOMALOHARIS_AUTO_DATA_DIR__:* ]]; then
    auto_data_paths="${remote_data_dir#__ANOMALOHARIS_AUTO_DATA_DIR__:}"
    legacy_data_dir="${auto_data_paths%%:*}"
    new_data_dir="${auto_data_paths#*:}"
    has_sqlite_data() {
        local directory="$1"
        [[ -f "$directory/sessions.sqlite3" \
            || -f "$directory/preset-models.sqlite3" \
            || -f "$directory/compute.sqlite3" ]]
    }
    if has_sqlite_data "$legacy_data_dir" && ! has_sqlite_data "$new_data_dir"; then
        remote_data_dir="$legacy_data_dir"
    else
        remote_data_dir="$new_data_dir"
    fi
fi
...
build_run_args() {
    ...
    if [[ "$include_mount" == "1" ]]; then
        ...
        run_args+=(--volume "$remote_data_dir/artifacts:/app/runtime-bundle/artifacts")
    fi
    run_args+=("$image_ref")
}
