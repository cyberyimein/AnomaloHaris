...
Environment:
  ...
  OUTPUT_DIR               Archive output directory. Default: artifacts/container-images
  ARCHIVE_PATH             Exact output archive path. Overrides OUTPUT_DIR naming
  DOCKERFILE               Dockerfile path. Default: docker/anomalo/Dockerfile
  INSTALL_EXTRAS           Python extras installed in image. Default: audio,buddy
...
DOCKERFILE="${DOCKERFILE:-$repo_root/docker/anomalo/Dockerfile}"
INSTALL_EXTRAS="${INSTALL_EXTRAS:-audio,buddy}"
